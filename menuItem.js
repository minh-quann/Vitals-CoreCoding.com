...
export const MenuItem = GObject.registerClass({
    ...
    // Called by PopupBaseMenuItem on any button release
    activate(event) {
        // Right-click: open group assignment dialog instead of toggling
        let currentEvent = Clutter.get_current_event();
        if (currentEvent && currentEvent.type() === Clutter.EventType.BUTTON_RELEASE && currentEvent.get_button() === 3) {
            this.emit('group-assign');
            return;
        }

        // Left-click: toggle sensor on/off in panel
