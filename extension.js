...
var VitalsMenuButton = GObject.registerClass({
    ...
    _init(extensionObject) {
        ...
        let settings = [ 'use-higher-precision', 'alphabetize', 'hide-zeros', 'fixed-widths', 'hide-icons', 'unit', 'memory-measurement', 'include-public-ip', 'network-speed-format', 'storage-measurement', 'include-static-info', 'include-static-gpu-info', 'show-group-labels', 'panel-groups' ];
    ...
    _createHotItem(key, value) {
        ...
        // Show icon only when: not using group labels AND icons not hidden, OR it's the default icon
        let useGroupLabels = this._settings.get_boolean('show-group-labels');
        if ((!useGroupLabels && !this._settings.get_boolean('hide-icons')) || key == '_default_icon_') {
    ...
    _drawMenu() {
        ...
        if (useGroupLabels) {
            // Group mode: read panel-groups and render sensors by group
            let groups = [];
            try {
                groups = JSON.parse(this._settings.get_string('panel-groups'));
            } catch (e) {
                groups = [];
            }
            ...
            for (let group of groups) {
                if (!group.name || !group.sensors || group.sensors.length === 0) continue;

                // Create the group text label
                this._createGroupLabel(group.name, group.name);

                // Create hot items for each sensor in the group
                for (let key of group.sensors) {
                    this._createHotItem(key);
                }
            }

            // Sync hot-sensors list from groups so dropdown checkmarks work correctly
            let allGroupSensors = [];
            for (let group of groups) {
                if (group.sensors) allGroupSensors.push(...group.sensors);
            }
            // Only update if different to avoid infinite loop
            let currentHot = this._settings.get_strv('hot-sensors');
            if (JSON.stringify(currentHot.sort()) !== JSON.stringify(allGroupSensors.sort())) {
                this._settings.set_strv('hot-sensors', allGroupSensors);
            }
        } else {
            // Normal mode: use hot-sensors flat list
    ...
    // Create a text label element for a sensor group on the panel bar
    _createGroupLabel(groupKey, text) {
        let labelKey = '__group_label_' + groupKey + '__';
        let item = new St.BoxLayout({
            style_class: 'vitals-panel-item vitals-group-label',
        });

        let label = new St.Label({
            style_class: 'vitals-panel-group-text',
            text: text,
            y_expand: true,
            y_align: Clutter.ActorAlign.CENTER
        });
    ...
    _appendMenuItem(sensor, key) {
        ...
        item.connect('toggle', (self) => {
            // Group mode: redirect left-click to group assignment dialog
            if (this._settings.get_boolean('show-group-labels')) {
                // Revert the toggle state (don't modify hot-sensors)
                self._checked = !self._checked;
                self._updateOrnament();
                // Open group dialog instead
                this.menu._getTopMenu().close();
                this._showGroupAssignDialog(self.key, self.label);
                return;
            }
        ...
        // right-click: open group assignment dialog
        item.connect('group-assign', (self) => {
            this.menu._getTopMenu().close();
            this._showGroupAssignDialog(self.key, self.label);
        });
    ...
    // Show a dialog to assign a sensor to a panel group
    _showGroupAssignDialog(sensorKey, sensorLabel) {
        let groups = this._getPanelGroups();

        let dialog = new ModalDialog.ModalDialog({ styleClass: 'vitals-group-dialog' });
        ...
        // Sensor name
        let sensorInfo = new St.Label({
            text: sensorLabel + '  (' + sensorKey + ')',
            style: 'font-size: 12px; color: #aaa; margin-bottom: 16px;'
        });
        dialog.contentLayout.add_child(sensorInfo);

        // Group buttons
        for (let i = 0; i < groups.length; i++) {
            let group = groups[i];
            let isInGroup = group.sensors && group.sensors.includes(sensorKey);

            let row = new St.BoxLayout({ style: 'spacing: 8px; margin-bottom: 6px;' });

            let checkLabel = new St.Label({
                text: isInGroup ? '  ✓  ' : '      ',
                style: 'font-size: 14px; color: #4da6ff; min-width: 30px;',
                y_align: Clutter.ActorAlign.CENTER
            });
            ...
            let delBtn = new St.Button({
                label: '✕',
                style_class: 'button',
                style: 'min-width: 30px; color: #ff6666;'
            });

            let groupIndex = i;
            btn.connect('clicked', () => {
                this._toggleSensorInGroup(sensorKey, groupIndex);
                dialog.close();
            });
        ...
        let sep = new St.Widget({
            style: 'height: 1px; background-color: #555; margin: 12px 0;',
            x_expand: true
        });
        dialog.contentLayout.add_child(sep);

        // New group creation row
        let newRow = new St.BoxLayout({ style: 'spacing: 8px;' });
        ...
        addBtn.connect('clicked', () => {
            ...
            if (name) {
                this._createNewGroupWithSensor(name, sensorKey);
                dialog.close();
            }
        });
        ...
        newRow.add_child(entry);
        newRow.add_child(addBtn);
        dialog.contentLayout.add_child(newRow);
    ...
    // Get panel groups from settings
    _getPanelGroups() {
        try {
    ...
    // Save panel groups to settings
    _savePanelGroups(groups) {
        this._settings.set_string('panel-groups', JSON.stringify(groups));
    }

    // Toggle a sensor in/out of a group
    _toggleSensorInGroup(sensorKey, groupIndex) {
        let groups = this._getPanelGroups();
        ...
        let idx = group.sensors.indexOf(sensorKey);
        if (idx >= 0) {
            // Remove from this group
            group.sensors.splice(idx, 1);
        } else {
            // Add to this group (remove from other groups first)
            for (let g of groups) {
                if (g.sensors) {
                    let pos = g.sensors.indexOf(sensorKey);
                    if (pos >= 0) g.sensors.splice(pos, 1);
                }
            }
            group.sensors.push(sensorKey);
        }

        this._savePanelGroups(groups);
        if (this._settings.get_boolean('show-group-labels')) {
            this._redrawMenu();
        }
    }

    // Create a new group and add a sensor to it
    _createNewGroupWithSensor(groupName, sensorKey) {
        let groups = this._getPanelGroups();

        // Remove sensor from other groups
        for (let g of groups) {
            if (g.sensors) {
                let pos = g.sensors.indexOf(sensorKey);
                if (pos >= 0) g.sensors.splice(pos, 1);
        ...
        groups.push({ name: groupName, sensors: [sensorKey] });
        this._savePanelGroups(groups);
        if (this._settings.get_boolean('show-group-labels')) {
            this._redrawMenu();
        }
